...
* `docs/engineering/ci.md` (見出し「**CI 品質ゲート方針**」でリポジトリ全体の CI 設計意図・責務分割・非対象を規定；ジョブ別コマンドは同ファイル「CI マトリックス」)
...
| テスト基盤 (PHPUnit + WorDBless) | 実装済み | 100 | - **ランタイム**: `phpunit/phpunit` **^13.1**、`phpunit.xml` は schema URL が PHPUnit **12.4** 系。<br>- **`composer.lock` をリポジトリ管理** し、`composer install` で WordPress 配置・WorDBless ドロップインまで再現可能。<br>- **`tests/bootstrap.php`** で WorDBless により WordPress 実ランタイムを読み込み。<br>- **テストスイート**: `unit` (`tests/Integration` 除外) と `wordpress` (統合のみ)。`composer test` は両スイート実行、`composer run test:unit` / `composer run test:wordpress` で個別実行。<br>- GitHub Actions 上のジョブ対応は下記「CI・品質ゲート」。追加の明確化なし |
| CI・品質ゲート (engineering / GitHub Actions) | 実装済み | 100 | - **規範**: **docs/engineering/ci.md** の見出し「**CI 品質ゲート方針**」(ゴールは契約整合・回帰検知・仕様乖離防止；品質ゲートは CI で自動実行し失敗時は merge 不可；ジョブは責務単位で分離；外部プロバイダ依存テストは CI に含めない)。責務・非責務・非対象 (プレビュー deployment・ブラウザマトリックス・負荷テスト等) は同セクション。<br>- **ワークフロー**: `.github/workflows/ci.yml` が同ファイル「CI マトリックス」と整合。**php-quality**: `test:unit` → `phpstan` → `phpcs`。**wordpress-integration**: `test:wordpress`。**openapi-codegen**: `npm ci` → `./scripts/generate/all.zsh` → `git diff --exit-code`。**typescript-sdk**: `npm test -w @s2j/similarity-client` → `build` → `typecheck` (いずれも `-w @s2j/similarity-client`)。Node **22.x**、PHP **8.3**。<br>- **静的解析**: `phpstan.neon` (level **3**、`src` / `adapters`)。<br>- **スタイル**: `phpcs.xml.dist` (**PSR-12**、`src` / `adapters` / `tests`；OpenAPI 生成 DTO `src/Contracts/DTO/Generated/` は対象外、tests は WorDBless 都合で PSR-1 の一部を緩和)。`composer run phpcbf` / `composer run lint:php`。<br>- **追加で詰められる余地** (実装％とは別): OpenAPI レスポンスの JSON Schema 機械検証を CI に載せるか、PHPCS の対象を生成 DTO に広げるかは engineering で確定 (ci.md の「CI で実行しないもの」とのバランス)。 |
...
* **CI とエンジニアリング品質**: **docs/engineering/ci.md** の「**CI 品質ゲート方針**」が merge 前に満たすべきゲートの規範であり、**.github/workflows/ci.yml** がそのジョブ実装です。PHP 側は **`composer run lint:php`** (PHPStan + PHPCS) と `test:unit` / `test:wordpress` / `phpcbf` が利用可能です。
...
* OpenAPI には `ErrorResponse` まで定義済みです。**HTTP レスポンスの JSON Schema 機械検証**を CI に載せるかどうかは、上表「REST API」「CI・品質ゲート」の任意 backlog と同様に仕様・engineering で確定します。
