...
| OpenAI Embeddings 連携 `OpenAIEmbeddingStrategy` | 実装済み | 98 | - **コード**: `endpoint` / `timeoutSeconds` をコンストラクタで注入可能 (`src/Infrastructure/Embedding/OpenAIEmbeddingStrategy.php`)。バッチ・エラー分岐は実装済み。<br>- **ドキュメント**: **usage_spec.md** にコンストラクタ (名前付き引数) ・設定表を反映済み。**README.md** に endpoint / timeout を一言でも追記すると利用者に親切 (任意)。 |
...
| README / 使用方法ドキュメントの整合 (命名・例コード) | 一部実装 | 96 | - **README.md**: PHP の `SimilarityService` / `EmbeddingService` / キャッシュは現行 `S2J\Similarity\...` で整合。**WordPress REST アダプタ**は論理パス・`/wp-json/s2j/v1/...`、`Routes::register`、Bearer、curl まで記載済み (**rest_api_spec** の必須導線と一致)。**TypeScript SDK** は利用・ビルド・`verify:codegen` を記載済み。<br>- **usage_spec.md**: コードベースと整合するよう更新済み (名前空間、`OpenAIEmbeddingStrategy` の API、例外型、`SimilarityService` のコンストラクタ、擬似コードと TS SDK の区分)。<br>- **100% の残り**: **README.md** に `OpenAIEmbeddingStrategy` のオプション引数を短く追記する (任意)。長大な usage_spec 全体の細部は運用で継続レビュー。 |
