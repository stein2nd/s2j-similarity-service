...
| HTTP サーバ実装 (WordPress REST API Adapter) | 実装済み | 80 | - **docs/interfaces/rest_api_spec.md**: 成立条件のうち routing/controller/error mapping/request validation/response serialization は `adapters/http/wordpress/` として実装済み<br>- **docs/interfaces/rest_api_spec.md**: 「HTTP integration test」を WordPress 実ランタイム上で満たす (WP REST Server を通す) テスト方針を確定し実装 |
| REST API (`POST /v1/similarity`, `POST /v1/embedding`) | 未実装 | 60 | - **docs/interfaces/rest_api_spec.md**: REST 成立条件 #6 の「HTTP integration test」を満たす (WP 実環境での疎通テスト)<br>- **docs/interfaces/rest_api_spec.md**: 認証 (Bearer token) の検証方法・権限モデルを確定 (現状は token 文字列一致の最小実装)<br>- **docs/interfaces/rest_api_spec.md**: レート制限の具体値 (単位/上限/Retry-After) を確定 |
| OpenAPI を Single Source にした codegen (TS types/Zod/PHP DTO) | 実装済み | 70 | - **codegen 実行基盤**: `scripts/generate/all.zsh` により `schema/openapi.yaml` から生成可能 (開発/CI/release のみで実行)<br>  - TS types/Zod/raw client (dev-only): `tools/generated/ts/`<br>  - PHP DTO (Composer 配布物に同梱): `src/Contracts/DTO/Generated/`<br>  - OpenAPI Generator 版の再現性: `openapitools.json` で固定<br>- **残課題 (100%条件)**: CI で `./scripts/generate/all.zsh` → `git diff --exit-code` を必須化し、生成差分があれば fail するルールを実装 (GitHub Actions 等)<br>- **公開範囲**: `docs/interfaces/sdk_spec.md` / `docs/overview.md` の方針どおり、TS の generated/raw は WordPress 利用者向け公開 API にしない (配布物に混入しない) ことを CI/リリース手順で担保 |
...
* HTTP runtime は WordPress REST API を想定し、`adapters/http/wordpress/` に routing/controller を実装済みです。
* `docs/interfaces/rest_api_spec.md` と `schema/openapi.yaml` は存在し、OpenAPI には `ErrorResponse` まで定義済みです。残課題は「HTTP integration test」「認証モデルの確定」「レート制限値の確定」です。
