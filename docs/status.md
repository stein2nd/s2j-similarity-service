...
* `docs/engineering/ci.md` (CI 品質ゲート・ジョブ構成の方針)
...
| CI・品質ゲート (engineering / GitHub Actions) | 実装済み | 100 | - **方針**: **docs/engineering/ci.md**。<br>- **ワークフロー**: `.github/workflows/ci.yml` に **php-quality** (`composer run test:unit` → `composer run phpstan` → `composer run phpcs`)、**wordpress-integration** (`composer run test:wordpress`)、**openapi-codegen** (`npm ci` → `./scripts/generate/all.zsh` → `git diff --exit-code`)、**typescript-sdk** (Vitest → build → typecheck) を定義。Node は **22.x**、PHP は **8.3**。<br>- **静的解析**: `phpstan.neon` (level **3**、`src` / `adapters`)。<br>- **スタイル**: `phpcs.xml.dist` (**PSR-12**、`src` / `adapters` / `tests`；OpenAPI 生成 DTO `src/Contracts/DTO/Generated/` は対象外、tests は WorDBless 都合で PSR-1 の一部を緩和)。`composer run phpcbf` で自動修正、`composer run lint:php` で PHPStan と PHPCS を連続実行。<br>- **外部プロバイダ API のライブテストは含めない** (ci.md の非対象どおり)。<br>- **追加で詰められる余地** (実装％とは別): OpenAPI レスポンスの JSON Schema 機械検証を CI に載せるか、PHPCS の対象を生成 DTO に広げるか (codegen 側の整形とセットか) は engineering で確定。 |
...
* **CI とエンジニアリング品質**: **docs/engineering/ci.md** を単一の方針ドキュメントとし、**.github/workflows/ci.yml** でジョブを分離しています。PHP 側は **`composer run lint:php`** (PHPStan + PHPCS) と個別スクリプト (`test:unit` / `test:wordpress` / `phpcbf`) が利用可能です。
