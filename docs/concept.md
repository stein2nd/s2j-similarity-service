...
* 外部の Embedding API (たとえば、OpenAI 等) を利用可能であること
...
外部 API 依存を最小化するため、以下の設計パターンを採用します。

#### Strategy パターン

* Embedding 取得処理を `EmbeddingStrategyInterface` に抽象化します。
* プロバイダごとに、実装を差し替え可能とします。
...
* 各 Strategy 実装は、外部 API をラップします。
* プロバイダ固有のレスポンスを、統一形式 (EmbeddingVector) に変換します。

### API キーの取り扱い

* API キーは、本ライブラリでは保持しません。
* コール側が管理し、Strategy 実装に注入します。

### 依存関係の原則

* 類似度の算出ロジックは、外部 API に依存しません。
* 外部 API 依存は、Strategy 実装に閉じ込めます。
...
* Strategy が、外部 API コールを担当します。
* Adapter が、データ変換を担当します。
* 両者は、同一クラス内に実装可能です。
...
* 外部 API コール、エラーハンドリング、リトライ制御などを個別に実装する負担が大きい
...
* 外部 API 依存を抽象化し、容易にプロバイダ差し替えできる
