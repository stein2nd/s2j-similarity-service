...
抽象インターフェースの重複を排除し、実装の一貫性を確保します。

### 設計方針 (規約)

* EmbeddingStrategyInterface を唯一の抽象とします。
* EmbeddingProvider という名称は、廃止します。

### 廃止ルール

* 新規コードで、EmbeddingProvider を使用してはならない
* 既存記述は、すべて EmbeddingStrategyInterface に置換する

### 補足

Strategy が「プロバイダの役割」を兼ねます。
...
異なる Embedding プロバイダ間の差異を明確化します。

### 設計方針 (規約)

* ベクトル次元は、プロバイダごとに異なる可能性があります。
* 異なるプロバイダ間での類似度計算は、非推奨とします。

### ルール

* 同一モデル・同一プロバイダ間でのみ、比較します。
...
キャッシュおよび再現性の、前提条件を明確にします。

### 設計方針 (規約)

* 同一入力・同一モデル・同一プロバイダにおいては、同一ベクトルが返ることを前提とします。
* モデル変更時は、結果が変化する可能性を許容します。
...
* キャッシュキーには、model / provider を含めます。
* モデル更新時は、キャッシュを無効化します。
